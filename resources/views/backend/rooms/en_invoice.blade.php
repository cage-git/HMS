...
<body>
@php
    $invTypeList = ['org'=>'ORIGINAL', 'dup'=>'DUPLICATE', 'cnl'=>'CANCELLED'];
    ...
    <div class="row">
        <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
            <table class="table table-bordered">
                <thead>
                <tr>
                    <th class="text-center" width="5%" colspan="2">{{lang_trans('txt_sno')}}.</th>
                ...
                @if($roomAmountGst>0)
                    <tr>
                        <th class="text-right" colspan="5">Tax ({{$gstPerc}} %)</th>
                        <td class="text-right">{{ numberFormat($roomAmountGst) }}</td>
                    </tr>
                    <tr>
                        <th class="text-right" colspan="5">CTax ({{$cgstPerc}} %)</th>
                ...
                @if($advancePayment>0)
                    <tr>
                        <th class="text-right" colspan="5">Advance Amount</th>
                ...
                @else
                    <tr>
                        <th class="text-right" colspan="5">Customer Balance {{($dueAmount < 0) ? '(Returned)':''}}</th>
                        <td class="text-right">{{ numberFormat(abs($dueAmount)) }}</td>
                    </tr>
                @endif
                @if($invoiceType == 'CANCELLED')
                    <tr>
                        <th class="text-right" colspan="5">Refund Amount</th>
                        <td class="text-right">{{ numberFormat($advancePayment) }}</td>
                    </tr>
                    <tr>
                        <th class="text-right" colspan="5">Balance Amount</th>
                ...
                @if($invoiceType != 'CANCELLED')
                    ...
                    <tr>
                        <td class="class-inv-17" colspan="6">
                            ...
                            @endphp
                            <img class="center-block mt-5" src="{!! @$generatedString->render() !!}">
                        </td>
                    </tr>
                @endif
                <tr>
    ...
    <div class="row">
        <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
            <table class="table table-bordered">
                ...
                @if($foodAmountGst>0)
                    <tr>
                        <th class="text-right" colspan="6">Tax ({{$gstPercFood}} %)</th>
                        <td class="text-right">{{ numberFormat($foodAmountGst) }}</td>
                    </tr>
                @endif
                @if($foodAmountCGst>0)
                    <tr>
                        <th class="text-right" colspan="6">CTax ({{$cgstPercFood}} %)</th>
