...
if(globalVar.page=='room_reservation_add'){
  ...
  $(document).on('click','.room_checkbox',function(){
    ...
    $(document).on('change','.room_price',function(){
        $.each($(".room_price"), function(){
            let textData = $(this).parent().find('a').text().trim().split('||');
            let newTextData;
            if(textData[1] == ')'){
                 newTextData = textData[0] +'||'+ $(this).val()+')';
            }else{
                 newTextData = $(this).parent().find('a').text().trim().replace(textData[1].replace(')', ''), $(this).val());
            }
            $(this).parent().find('a').text(newTextData);
            if($(this).val() == ''){
                $(this).val($(this).attr("data-original")).trigger('change')
            }
        });

    });
    $(document).on('change','.calculate_total_amount',function(){
        let price = 0;
        $.each($(".room_checkbox:checked"), function(){
            var getId = $(this).val().split('~')[0];
            price = price + (($('input[name="roomtype_'+getId+'"]').val()) * $('#duration_of_stay').val())
        });
        $('#total_amount_show').text(price);
    })
