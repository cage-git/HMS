...
class ReportController extends Controller
{
    ...
    public function searchCheckouts(Request $request) {
        ...
        select('reservations.*')->
        whereStatus(1)->where('reservations.is_deleted',0)->whereIsCheckout(1)->orderBy('reservations.created_at','DESC')->where('reservations.business_id', Auth::user()->business_id);
        }else{
            $query = Reservation::
        leftjoin('customers as c', 'c.id', '=', 'reservations.customer_id')->
        select('reservations.*')->
